...
# filepath = os.getcwd() + "/Certificates/DataBase/DataBase.xlsx"
...
def combine_subjects_sheets():
    folder_path = 'Subjects_Sheets/' + TermFolder_combobox.get() + '/' + MonthFolder_combobox.get()
    if os.path.exists(folder_path):
        ...
        output_path = f'Combined_Data_{TermFolder_combobox.get()}_{MonthFolder_combobox.get()}.xlsx'
        combined_data.to_excel(output_path, index=False)
        tkinter.messagebox.showinfo(title='Sheets Combined successfully',
                                    message='Sheet Created with at the following path:' + os.getcwd() + f'/Combined_Data_{TermFolder_combobox.get()}_{MonthFolder_combobox.get()}.xlsx')
...
def Create_sheets_per_subject():
    ...
    month = MonthExtract_combobox.get()
    ...
    for value in distinct_values['SubjectId']:
        ...
        Class = sqldf(f"SELECT DISTINCT Class FROM dff WHERE Id = '{value}'")
        Class2 = Class.to_string(index=1).replace("Class\n0", "").replace(" ", "")

        selected_rows = sqldf(f"SELECT * FROM df WHERE SubjectId = '{value}' AND Month = '{month}' ORDER BY QuizType, StudentId ASC")
        ...
        if not os.path.exists(f"Subjects_Sheets/Term_{term}/Month_{month}"):
            os.makedirs(f"Subjects_Sheets/Term_{term}/Month_{month}")
        if not os.path.exists(f"Subjects_Sheets/Term_{term}/Month_{month}/Grade_{GradeId2}"):
            os.makedirs(f"Subjects_Sheets/Term_{term}/Month_{month}/Grade_{GradeId2}")
        if not os.path.exists(f"Subjects_Sheets/Term_{term}/Month_{month}/Grade_{GradeId2}/Language Type_{LanguageType2}"):
            os.makedirs(f"Subjects_Sheets/Term_{term}/Month_{month}/Grade_{GradeId2}/Language Type_{LanguageType2}")
        if not os.path.exists(f"Subjects_Sheets/Term_{term}/Month_{month}/Grade_{GradeId2}/Language Type_{LanguageType2}/Class_{Class2}"):
            os.makedirs(f"Subjects_Sheets/Term_{term}/Month_{month}/Grade_{GradeId2}/Language Type_{LanguageType2}/Class_{Class2}")

        selected_rows.to_excel(
            f"Subjects_Sheets/Term_{term}/Month_{month}/Grade_{GradeId2}/Language Type_{LanguageType2}/Class_{Class2}/{subjectName2}_Id_{value}.xlsx",
            index=False)
        ...
        # worksheet.column_dimensions[openpyxl.utils.get_column_letter(11)].protection = Protection(locked=False)
        for row in worksheet.iter_rows(min_col=11, max_col=11):
            for cell in row:
                cell.protection = Protection(locked=False)
        worksheet.protection.sheet = True
        worksheet.protection.set_password('ahmedhassanfgs')
        ...
        workbook.save(
            f"Subjects_Sheets/Term_{term}/Month_{month}/Grade_{GradeId2}/Language Type_{LanguageType2}/Class_{Class2}/{subjectName2}_Id_{value}.xlsx")
        color_rows(f"Subjects_Sheets/Term_{term}/Month_{month}/Grade_{GradeId2}/Language Type_{LanguageType2}/Class_{Class2}/{subjectName2}_Id_{value}.xlsx")

        print(
            "Created:" + f"Subjects_Sheets/Term_{term}/Month_{month}/Grade_{GradeId2}/Language Type_{LanguageType2}/Class_{Class2}/{subjectName2}_Id_{value}.xlsx")
...
MonthExtract_label = tkinter.Label(tab3, text="Month")
MonthExtract_combobox = ttk.Combobox(tab3, values=["1", "2", "3"], state="readonly")
MonthExtract_label.grid(row=2, column=0)
MonthExtract_combobox.grid(row=3, column=0)

output_label = tkinter.Label(tab3,
                             text="Output File name (Without extension), file with exist under this path:" + os.getcwd()+"/#FileName#.xlsx")
output_label.grid(row=4, column=0)
output_entry = tkinter.Entry(tab3)
output_entry.grid(row=5, column=0)
...
Notes_label = tkinter.Label(tab3,
                                            text="Important Notes Before run:\n 1. Database file should be added to the following path: " + os.getcwd()+"/ with name \"DataBase.xlsx\"\n 2. After the progress bar is completed you can find the generated sheets per subject in" + os.getcwd()+"/SubjectsSheets")
...
Hint_label = tkinter.Label(tab4,
                           text="Important Notes, 1. this process should be done after running Split sheets per subjects and fill all marks, then put the sheets back each one in it's place.\n2. Subjects sheets should be reviewed carefully before merge as marks must contain only numbers or empty not any other inputs")
...
MonthFolder_label = tkinter.Label(tab4, text="Month Folder")
MonthFolder_combobox = ttk.Combobox(tab4, values=["Month_1", "Month_2", "Month_3"], state="readonly")
MonthFolder_label.grid(row=3, column=0)
MonthFolder_combobox.grid(row=4, column=0)


Create_Final_Marks_sheet_button = tkinter.Button(tab4, text="Combine sheets", command=combine_subjects_sheets,
                                                 state="normal")
Create_Final_Marks_sheet_button.grid(row=5, column=0, sticky="news", padx=20, pady=10)

Notes_label = tkinter.Label(tab4,
                            text="Final combined data can be found in the following sheet" + os.getcwd() + "/Combined_Data_Term#_Month# , please overwrite the data to Exams sheet in DataBase.xlsx file")
